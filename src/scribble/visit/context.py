...
class Context(AbstractContext):
    ...
    # {
        ...
        #
        # FIXME: Global protocol reachability (no continuations after a continue)
        # -- will be moved to reachability in local projections (more
        # accurately, will be part of local projection reachability)
        #
        ...
        # Map from member name to boolean
        #
    ...
    def add_projection(self, fullname, LocalProtocolDecl):
        clone = self.clone()
        clone._projections[fullname] = LocalProtocolDecl
        return clone
    ...
    def push_globalparallel(self, node_):
        ...
        _append_disabled_frame(clone._rec_labs)
        # Equivalent to above recLabs treatment, adapted for recursive-do?
        # FIXME: not currently specified in the langref
        _append_disabled_frame(clone._do_stack)
        return clone
