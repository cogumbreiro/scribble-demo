...
class ScopeAdder(Traverser):
    ...
    def _visit_globaldo(self, node_):
        """_scope = globaldo.get_scope(node_)
        if _scope == globaldo.EMPTY_SCOPE_NAME:
            children = []
            scopenode = globaldo.get_scope_child(node_)
            children.append(self._replace_scope_node(scopenode_))
            children.append(globaldo.get_argumentlist_child(node_))
            children.append(globaldo.get_roleinstantiationlist_child(node_))
            children.extend(globaldo.get_target_name_children(node_))
            node = util.antlr_dupnode_and_replace_children(node, children)"""
        return node_
