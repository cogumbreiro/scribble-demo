...
def check_wellformedness(checker, target, node_):
    ...
    tree = context.get_visible_global(target)
    ...
    #{
        ...
        if kind == constants.KIND_MESSAGE_SIGNATURE:
            ...
            if util.get_node_type(arg) == constants.MESSAGE_SIGNATURE_NODE_TYPE:
              ...
              # Section 4.6.3 -- the argument should be a bound sig parameter
                ...
                if param not in context.get_parameters().keys():
                    # The argument isn't a valid parameter
                    util.report_error("Bad argument : " + param)
                #if context.get_parameter(param) != constants.SIGKW:
                if context.get_parameter(param) != \
        ...
        # Section 4.6.3 -- case (2): the target parameter is a type parameter
            ...
            if val not in context.get_visible_payloads():
                # FIXME: factor out with payload
                #if context.get_current_module() + '.' + val in context.get_visible_payloads():
                if val not in context.get_parameters().keys():
                    util.report_error("Bad argument: " + val)
                ##if not(context.get_parameter(val) == constants.TYPEKW):
                if context.get_parameter(val) != constants.KIND_PAYLOAD_TYPE:
...
def is_empty(node):
    return util.get_node_type(node) == EMPTY_ARGUMENT_LIST


def pretty_print(node):
    args = get_argument_children(node)
    text = ""
    if args:
